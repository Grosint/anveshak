...
description: "Content ingest quality and relevance filtering. Covers quality gates at every consumption point, unicode-aware word counting for Devanagari/Arabic/CJK, topic relevance gates before clustering, per-topic threshold auto-calibration, RSS paywall validation, credibility flow, and NLP results in labels JSONB. Use when working on scraping, ingest, content filtering, relevance thresholds, or quality scoring."
---

# Content Quality & Relevance Filtering

14 learned instincts. All content processing pipelines, incl. language detection + NLP enrichment.
...
- Quality signal computed → apply filter at EVERY consumption point, not one
  ...
  See: `.claude/skills/learned/quality-gate-all-consumers.md`
...
- Word-counting regex must cover all scripts:
  ...
  See: `.claude/skills/learned/quality-gate-unicode-ranges.md`
...
- After embedding, compute cosine similarity against topic query embedding
  ...
  See: `.claude/skills/learned/post-embedding-relevance-gate.md`
...
- `detect_language()` must return real detected language, not filter by downstream model support (spaCy, NLLB)
  ...
  See: `.claude/skills/learned/detect-language-must-not-gatekeep.md`
...
- Topic content queries must include BOTH `topic_id = $1` AND
  ...
  See: `.claude/skills/learned/cross-topic-join-table-queries.md`
...
- Global threshold fails for mixed-breadth topics (IOR median 0.14 vs LAC 0.26)
  ...
  See: `.claude/skills/learned/per-topic-relevance-auto-calibration.md`
...
- In-memory URL set per scrape job skips duplicate media downloads
  ...
  See: `.claude/skills/learned/redis-url-dedup-sha256-ttl.md`, `.claude/skills/learned/url-level-media-dedup.md`
...
- Fetching full article from RSS: validate content BEFORE replacing summary
  ...
  Bypass clean/raw ratio check if `clean_text >= 500 chars` (HTML-heavy sites hit 0.06-0.12 on real articles)
  MUST fire AFTER paywall + nav-icon gates — 500-char paywall page still garbage
  Below 500 chars the ratio check is useful (short text from heavy page = likely nav fragment)
  See: `.claude/skills/learned/rss-fetch-paywall-validation.md`, `.claude/skills/learned/quality-ratio-bypass-on-length.md`
...
- Sentiment + keywords stored in `content_items.labels` JSONB (not separate columns)
  ...
  See: `.claude/skills/learned/nlp-results-in-jsonb-labels.md`, `.claude/skills/learned/jsonb-labels-api-surfacing.md`
