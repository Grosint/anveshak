...
class TestOllamaCircuitBreaker:
    ...
    async def test_blocks_call_when_open(self):
        ...
        redis.get = AsyncMock(
            side_effect=lambda k: "3" if "failures" in k else str(time.monotonic())
        )
...
class TestCreatePoolWithRetry:
    ...
    async def test_raises_after_max_retries(self):
        ...
        with patch("services.api.anveshak.api.db.pool.asyncpg") as mock_asyncpg:
            mock_asyncpg.create_pool = AsyncMock(side_effect=ConnectionError("refused"))
...
class TestAlertmanagerCompose:
    ...
    def test_alertmanager_service_exists(self):
        ...
        assert "alertmanager" in content.lower(), "compose.yml must include an alertmanager service"
...
class TestFrontendExportButtons:
    ...
    def test_content_feed_has_export(self):
        """The content feed must offer an export affordance.

        The feed used to be a standalone ContentFeed component; it now lives
        in the content view of TopicWorkspace. Assert on the endpoint the
        button targets, not on a filename, so a rename does not silently
        stop testing anything.
        """
        from pathlib import Path

        page = Path("frontend/src/pages/TopicWorkspace.tsx")
        assert page.exists(), f"content feed page missing: {page}"

        content = page.read_text()
        assert "ExportButton" in content, "content feed must render ExportButton"
        assert "/api/v1/export/content" in content, (
            "content feed export must target the content export endpoint"
        )
