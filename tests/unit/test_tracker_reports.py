...
class TestTrackerReportSQL:
    ...
    def test_fetch_tracker_rag_chunks_sql_exists(self):
        ...
        assert "confirmed" in sql, "Must only include confirmed items (not pending)"
        assert "credibility_score_at_capture" in sql, "Must filter by credibility threshold"
...
class TestReportInsertWithTracker:
    ...
    def test_sql_insert_report_has_tracker_id(self):
        ...
        assert "tracker_id" in sql, "SQL_INSERT_REPORT must include tracker_id column"
    ...
    def test_insert_report_accepts_tracker_id(self):
        ...
        assert "tracker_id" in sig.parameters, "insert_report() must accept tracker_id parameter"
...
class TestTrackerReportsRoute:
    ...
    def test_tracker_reports_route_exists(self):
        ...
        assert "reports" in source.lower(), "trackers routes must include a reports endpoint"
