...
class TestWorstCaseScore:
    ...
    def test_empty_returns_none(self):
        """[] -> None (AGENTS.md rule 7: None on failure, never 0.0)."""
        from anveshak.vision.video import worst_case_score
...
class TestExtractKeyframes:
    ...
    async def test_ffmpeg_fallback(self):
        ...
        with (
            patch("anveshak.vision.video._FFMPEG_AVAILABLE", None),
            patch("shutil.which", return_value=None),
            patch(
                "anveshak.vision.video._pillow_first_frame", new_callable=AsyncMock
            ) as mock_pillow,
        ):
