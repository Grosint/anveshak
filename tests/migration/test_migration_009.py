...
async def test_scam_templates_has_expected_columns(db_pool):
    ...
    expected = {
        "id", "org_id", "name", "display", "category", "keywords",
        "min_keyword_hits", "expected_identifiers", "severity",
        "reference_embedding", "legal_sections", "is_builtin", "is_active",
...
async def test_scam_templates_category_check(db_pool):
    ...
    assert category_check, (
        f"scam_templates must have CHECK constraint on category. "
        f"Found constraints: {defs}"
    )
...
async def test_scam_templates_severity_check(db_pool):
    ...
    assert severity_check, (
        f"scam_templates must have CHECK constraint on severity. "
        f"Found constraints: {defs}"
    )


# ---------------------------------------------------------------------------
# scam_templates — labels non-optional (CLAUDE.md rule 2)
# ---------------------------------------------------------------------------

async def test_scam_templates_labels_not_nullable(db_pool):
    """scam_templates.labels must be NOT NULL (CLAUDE.md rule 2)."""
    async with db_pool.acquire() as conn:
...
async def test_scam_templates_reference_embedding_type(db_pool):
    ...
    assert udt == "vector", (
        f"reference_embedding must be vector type, got '{udt}'"
    )
...
async def test_identifier_clusters_labels_not_nullable(db_pool):
    """identifier_clusters.labels must be NOT NULL (CLAUDE.md rule 2)."""
    async with db_pool.acquire() as conn:
        is_nullable = await conn.fetchval(
            "SELECT is_nullable FROM information_schema.columns "
...
async def test_identifier_clusters_source_count_index(db_pool):
    ...
    assert "source_count" in rows[0]["indexdef"], (
        "idx_ic_source_count must index source_count"
    )
...
async def test_topics_identifier_signal_threshold_default(db_pool):
    ...
    assert "2" in str(default), (
        f"identifier_signal_threshold default must be 2, got '{default}'"
    )
...
async def test_builtin_templates_seeded(db_pool):
    ...
    async with db_pool.acquire() as conn:
        count = await conn.fetchval(
            "SELECT COUNT(*) FROM scam_templates WHERE is_builtin = true"
        )
    assert count == 11, (
        f"Expected 11 built-in templates, got {count}"
    )
...
async def test_builtin_templates_have_null_org_id(db_pool):
    ...
    async with db_pool.acquire() as conn:
        non_null = await conn.fetchval(
            "SELECT COUNT(*) FROM scam_templates "
            "WHERE is_builtin = true AND org_id IS NOT NULL"
        )
...
async def test_builtin_template_names(db_pool):
    ...
    async with db_pool.acquire() as conn:
        rows = await conn.fetch(
            "SELECT name FROM scam_templates WHERE is_builtin = true"
        )
...
async def test_builtin_templates_have_valid_severity(db_pool):
    ...
    async with db_pool.acquire() as conn:
        rows = await conn.fetch(
            "SELECT name, severity FROM scam_templates WHERE is_builtin = true"
        )
...
async def test_builtin_templates_have_valid_category(db_pool):
    ...
    async with db_pool.acquire() as conn:
        rows = await conn.fetch(
            "SELECT name, category FROM scam_templates WHERE is_builtin = true"
        )
