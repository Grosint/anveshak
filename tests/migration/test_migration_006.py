...
async def test_source_catalog_has_expected_columns(db):
    ...
    expected = {
        ...
        "recommendation_rank", "created_at", "updated_at", "labels",
    }
    async with db.acquire() as conn:
...
async def test_catalog_approvals_has_expected_columns(db):
    ...
    expected = {
        "catalog_entry_id", "topic_id", "source_id",
        "approved_by", "approved_at", "labels",
    }
    async with db.acquire() as conn:
        rows = await conn.fetch(
            "SELECT column_name FROM information_schema.columns "
...
async def test_discovered_sources_has_expected_columns(db):
    ...
    expected = {
        "id", "topic_id", "domain_or_handle", "platform",
        "discovery_method", "citation_count", "confidence_score",
        "evidence", "status", "source_id",
...
async def test_content_items_has_forwarding_columns(db):
    ...
    assert expected == actual, f"content_items forwarding columns: expected {expected}, got {actual}"
