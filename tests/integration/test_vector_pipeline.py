...
async def vector_topic(db_pool):
    ...
    async with db_pool.acquire() as conn:
        await conn.execute(
            "DELETE FROM near_duplicates WHERE content_item_a_id IN (SELECT id FROM content_items WHERE topic_id=$1) OR content_item_b_id IN (SELECT id FROM content_items WHERE topic_id=$1)",
            topic_id,
        )
...
async def sources_three(db_pool):
    ...
    ids = {}
    platforms = {
        "telegram": "t.me/vector_test",
        "reddit": "r/VectorTest",
        "web": "https://vectortest.com",
    }
...
async def test_dedup_prevents_false_signal(db_pool, vector_topic, sources_three):
    ...
    assert len(pairs) >= 2, (
        f"Expected ≥2 near-duplicate pairs from 3 identical embeddings, got {len(pairs)}"
    )
...
async def test_temporal_window_excludes_old_items(db_pool, vector_topic, sources_three):
    ...
    for i in range(2):
        await insert_item(
            ...
            vector_topic,
            source_id,
            f"Old content {i} {uuid.uuid4()}",
            base_vec,
            captured_at=old_time,
        )
    for i in range(2):
        await insert_item(
            db_pool, vector_topic, source_id, f"Fresh content {i} {uuid.uuid4()}", base_vec
        )
...
async def test_label_staleness_detects_composition_change(db_pool, vector_topic, sources_three):
    ...
    for i in range(3):
        ...
        iid = await insert_item(
            db_pool, vector_topic, source_id, f"Label staleness test {i} {uuid.uuid4()}", vec
        )
        item_ids.append(iid)
        ...
        for tid in topic_ids:
            for i in range(3):
                ...
                await insert_item(
                    ...
                    tid,
                    source_id,
                    f"Convergence test {tid[:8]} item {i} {uuid.uuid4()}",
                    vec,
                )
...
async def test_full_pipeline_chain(db_pool, vector_topic, sources_three):
    ...
    for platform, source_id in sources_three.items():
        for i in range(2):
            noise = (
                np.random.RandomState(abs(hash(platform)) % (2**32) + i)
                .randn(384)
                .astype(np.float32)
                * 0.02
            )
            ...
            await insert_item(
                ...
                vector_topic,
                source_id,
                f"Full chain test {platform} {i} {uuid.uuid4()}",
                vec,
            )
