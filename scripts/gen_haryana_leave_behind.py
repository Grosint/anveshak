#!/usr/bin/env python3
"""Generate a leave-behind PDF for the Haryana STF demo.
...
TOPIC_ID = "hr-stf-001"
OUTPUT_PATH = "/tmp/haryana_leave_behind.pdf"
...
_LANG_NAMES = {
    ...
    "ta": "Tamil", "kn": "Kannada", "mr": "Marathi", "bn": "Bengali",
    "zh": "Chinese", "ar": "Arabic", "ru": "Russian", "pa": "Punjabi",
    "gu": "Gujarati", "ne": "Nepali", "as": "Assamese",
}

_TYPE_OVERRIDES = {
    "panchkula": "GPE", "ambala": "GPE", "karnal": "GPE",
    "hisar": "GPE", "rohtak": "GPE", "sirsa": "GPE",
    "yamunanagar": "GPE", "kurukshetra": "GPE",
    "sonipat": "GPE", "panipat": "GPE",
}
...
async def main() -> None:
    ...
    bluf = (
        ...
        f"{stats['source_count']} sources across RSS news, Telegram intelligence "
        f"channels, official government sources, and dark web reveals "
        f"{stats['cluster_count']} narrative clusters and {stats['signal_count']} "
        f"intelligence signals related to drug trafficking activity in Haryana. "
        f"Key findings: Punjab-Haryana drug corridor remains the primary transit "
        f"route for Afghan heroin to Delhi-NCR; chitta epidemic in Ambala-Karnal "
        f"belt causing youth fatalities; cross-border drone-based smuggling "
        f"increasing. Engine C identifier intelligence extracted phone numbers "
        f"and Telegram handles from unstructured content, linking identifiers "
        f"across multiple independent sources — automating cross-source "
        f"correlation that would require manual search across hundreds of messages. "
    ...
    report_data = {
        "id": "hr-leave-behind-001",
        "topic_name": "Haryana STF Narcotics Intelligence — Briefing",
        "report_type": "research_summary",
        "generated_at": now.isoformat(),
        "confidence_score": 0.74,
        "content_item_count": stats["content_count"],
        "labels": {"classification": "RESTRICTED", "domain": "narcotics"},
        "topic_stats": {
    ...
    path = await generate_pdf(
        report_id="haryana-leave-behind",
