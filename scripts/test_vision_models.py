...
def test_yolo_loads() -> dict:
    ...
    try:
        ...
        return _result("yolo_load_and_detect", True,
                        f"model loaded, {len(detections)} detections on test image", elapsed)
...
def test_clip_loads() -> dict:
    ...
    try:
        ...
        return _result("clip_load_and_classify", has_scores,
                        f"model loaded, {len(results)} categories scored", elapsed)
    except Exception as exc:
...
def test_deepfake_image_score() -> dict:
    ...
    try:
        ...
        detector = get_deepfake_detector(settings.vision_deepfake_image_model, settings.vision_device)
        image_bytes = _make_test_image()
        score = detector.score(image_bytes)
        elapsed = time.monotonic() - t0
        ok = isinstance(score, float) and 0.0 <= score <= 1.0
        return _result("deepfake_image_score", ok,
                        f"score={score:.4f}, type={type(score).__name__}", elapsed)
    except Exception as exc:
...
def test_deepfake_video_score() -> dict:
    ...
    try:
        from anveshak.vision.detectors import get_deepfake_detector
        from anveshak.vision.settings import settings
        detector = get_deepfake_detector(settings.vision_deepfake_video_model, settings.vision_device)
        image_bytes = _make_test_image()
        score = detector.score(image_bytes)
        elapsed = time.monotonic() - t0
        ok = isinstance(score, float) and 0.0 <= score <= 1.0
        return _result("deepfake_video_score", ok,
                        f"score={score:.4f}, type={type(score).__name__}", elapsed)
    except Exception as exc:
...
def test_deepfake_score_not_bool() -> dict:
    """Deepfake score must be float, never bool (CLAUDE.md rule 7)."""
    t0 = time.monotonic()
    try:
        from anveshak.vision.detectors import get_deepfake_detector
        from anveshak.vision.settings import settings
        ...
        return _result("deepfake_not_bool", ok,
                        f"type={type(score).__name__}, value={score}", elapsed)
    except Exception as exc:
...
def main():
    results = []
    for test_fn in [test_yolo_loads, test_clip_loads, test_deepfake_image_score,
                    test_deepfake_video_score, test_deepfake_score_not_bool]:
