...
def check_ollama() -> Check:
    ...
    if not models:
        ...
        return Check(
            "Ollama",
            False,
            f"no models loaded — run: docker exec anveshak-ollama ollama pull {configured_model}",
        )
...
def login() -> tuple[Check, str | None]:
    ...
    passed = bool(token)
    return Check(
        "Login", passed, "OK" if passed else "no access_token — check seed_demo.sql is loaded"
    ), token
...
def check_corpus(metrics: dict) -> list[Check]:
    ...
    checks.append(
        Check(
            "Embedding coverage",
            pct >= 80.0,
            f"{embedded}/{total} embedded ({pct:.1f}%) — need ≥80%"
            if pct < 80
            else f"{embedded}/{total} ({pct:.1f}%) ✓",
        )
    )
    ...
    checks.append(
        Check(
            ...
            last_24h >= 1,
            f"{last_24h} items in last 24h — pipeline may be stalled (check scraper/social logs)"
            if last_24h == 0
...
def check_intelligence(metrics: dict) -> list[Check]:
    ...
    checks.append(
        Check(
            "Narrative clusters",
            clusters >= 1,
            f"{clusters} clusters (need ≥1 — analyst worker must have run)"
            if clusters == 0
            else f"{clusters} clusters ✓",
        )
    )
    ...
    checks.append(
        Check(
            ...
            signals >= 1,
            f"{signals} signals in last 30d — no threshold crossings yet"
            if signals == 0
            else f"{signals} signal(s) ✓",
        )
    )
...
def check_reports(metrics: dict) -> list[Check]:
    ...
    return [
        Check(
            ...
            reports >= 1,
            f"{reports} — trigger one via Reports page once corpus has ≥20 items"
            if reports == 0
...
def check_sources(metrics: dict) -> list[Check]:
    ...
    checks.append(
        Check(
            "Active sources",
            active >= 1,
            f"{active} active sources (need ≥1 — add sources via Sources page)"
            if active == 0
    ...
    checks.append(
        Check(
            "Sources down",
            True,
            f"{down} source(s) currently down — check Sources page for details"
            if down > 0
            else "0 sources down ✓",
            warning=(down > 0),
        )
...
def check_multilingual(metrics: dict) -> list[Check]:
    ...
    checks.append(
        Check(
            ...
            + (
                " — seed Chinese sources: uv run python scripts/seed_chinese_sources.py"
                if zh_items == 0
                else " ✓"
            ),
            warning=(zh_items == 0),
    ...
    if zh_items > 0:
        ...
        checks.append(
            Check(
                ...
                if pct < 80
                else f"{translated}/{zh_items} translated ({pct:.1f}%) ✓",
            )
        )
    else:
        checks.append(
    ...
    checks.append(
        Check(
            ...
            + (
                " — analyst worker may not have run yet"
                if zh_entities == 0 and zh_items > 0
                else ""
            )
    ...
    try:
        ...
        checks.append(
            Check(
                ...
                "running ✓"
                if loop_running
                else "not found in scheduler logs — check scheduler rebuild",
