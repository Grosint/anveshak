...
- Load ML models once per worker via module-level lazy globals
  ...
  See: `.agents/skills/learned/references/arq-worker-ml-singleton.md`
...
- Split into lightweight scheduler (512 MiB) + heavy worker (6 GiB)
  ...
  See: `.agents/skills/learned/references/scheduler-worker-split.md`
...
- Every service inserting content_items MUST enqueue `analyse_content` directly
  ...
  See: `.agents/skills/learned/references/scraper-must-enqueue-not-rely-on-sweep.md`
...
- Circuit breaker: skip unhealthy sources via SQL `health_status` column
  ...
  See: `.agents/skills/learned/references/circuit-breaker-sql-filter.md`
...
- One-time writes: sentinel `WHERE generated_at IS NULL`
  ...
  See: `.agents/skills/learned/references/immutable-write-idempotency.md`
...
- Flush ARQ results from Redis at benchmark cleanup
  ...
  See: `.agents/skills/learned/references/benchmark-arq-dedup-flush.md`