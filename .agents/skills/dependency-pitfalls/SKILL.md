---
name: dependency-pitfalls
description: "Library pitfalls and uv workspace structure. Covers the asyncpg JSONB codec on a shared pool, Jinja2 get returning None, optional dependency lazy imports, passlib breaking with bcrypt 4, SDK dependency boundaries, uv workspace restructure order, and Vitest config placement. Use when adding a dependency, hitting a library error, or restructuring the workspace."
---
...
- asyncpg returns JSONB as `str` by default. ONE shared pool factory (`sdk/anveshak/db.py:create_db_pool()`) w/ codec registration.
  ...
  See: `.agents/skills/learned/references/asyncpg-jsonb-codec-shared-pool.md`
...
- `.get(key, default)` returns `None` when key exists w/ None value — NOT the default.
  ...
  See: `.agents/skills/learned/references/jinja2-none-vs-missing-default.md`
...
- Heavy/optional deps (WeasyPrint, PyMuPDF, facetorch): module-level try/except, `None` sentinel.
  ...
  See: `.agents/skills/learned/references/optional-dep-lazy-import-two-level-log.md`
...
- Shared utilities in SDK = dependency-free. No asyncpg, no arq, no service-specific imports.
  ...
  See: `.agents/skills/learned/references/sdk-shared-utility-no-db.md`
...
- Safe sequence: move files -> delete old -> update pyproject.toml `packages` -> update Dockerfiles -> `uv sync` -> run tests.
  ...
  See: `.agents/skills/learned/references/uv-workspace-restructure.md`
...
- Config in `vite.config.ts` `test:` block, NOT separate vitest config. Setup file: `src/test/setup.ts` w/ jest-dom.
  ...
  See: `.agents/skills/learned/references/vitest-vite-setup.md`
