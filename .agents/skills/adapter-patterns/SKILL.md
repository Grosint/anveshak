...
description: "Source adapter lifecycle and resilience. Covers authenticate converting every exception to AdapterAuthError, Redis-backed per-adapter circuit breakers, credential refresh before circuit break, startup credential validation, and webhook fire-and-forget. Use when writing or debugging a source adapter."
---
...
- Every adapter's `authenticate()` converts ALL platform-specific exceptions to `AdapterAuthError`
  ...
  See: `.agents/skills/learned/references/adapter-auth-must-catch-connection-errors.md`
...
- Three-state: CLOSED → OPEN (N consecutive failures) → HALF_OPEN (after cooldown probe)
  ...
  See: `.agents/skills/learned/references/adapter-circuit-breaker-redis.md`
...
- On `AdapterAuthError` during `collect()`, attempt `refresh_credentials()` BEFORE recording failure
  ...
  See: `.agents/skills/learned/references/credential-refresh-before-circuit-break.md`
...
- Validate required env vars BEFORE adapter instantiation, not at first API call
  ...
  See: `.agents/skills/learned/references/startup-credential-validation.md`
...
- Secondary notifications (webhook, email) NEVER block primary delivery (WebSocket)
  ...
  See: `.agents/skills/learned/references/webhook-fire-and-forget.md`
